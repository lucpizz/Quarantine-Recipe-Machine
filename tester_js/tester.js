...
// -----------------------------------------------
      ...
      function createIngredientParameters(ingredient_0, ingredient_1, separator, foodSite ){
           
        for(i = 0; i < 2; i++ ){ // *** <-- Just change iCount in "i<2" for igredient addition.
           // Find the value of each search item..
           let nameGenerate = eval("ingredient_".concat(i));
           
           if(nameGenerate != "" && nameGenerate != undefined && nameGenerate != null){
                     if(ingredients ===""){  // If ingredients is empty, include with no comma
